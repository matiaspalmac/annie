    ...
    if (itemSeleccionado === "cebo_simple") {
      ...
      await db.execute({
        sql: `INSERT INTO inventario_economia (user_id, item_id, cantidad)
              VALUES (?, ?, 3)
              ON CONFLICT(user_id, item_id) DO UPDATE SET cantidad = inventario_economia.cantidad + 3`,
        args: [interaction.user.id, itemSeleccionado],
      });

      return { ok: true, message: "🎣 Compraste **Cebo Simple x3**. Se usará automáticamente al pescar para mejorar tus capturas." };
    }
    ...
    if (itemSeleccionado === "seguro_antirobo_24h") {
      ...
      await db.execute({
        sql: `UPDATE usuarios
              ...
              END
              WHERE id = ?`,
        args: [ahora, duracion, ahora + duracion, interaction.user.id],
      });
