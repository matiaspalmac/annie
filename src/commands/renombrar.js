...
export const data = new SlashCommandBuilder()
  .setName("renombrar")
  .setDescription("Renombra elementos especiales, como tus mascotas.")
  .addStringOption((option) =>
    option
      .setName("opcion")
      .setDescription("Qué quieres renombrar")
      .setRequired(true)
      .addChoices({ name: "mascota", value: "mascota" })
  )
  .addStringOption((option) =>
    option
      .setName("mascota")
...
export async function execute(interaction, bostezo) {
  ...
  try {
    if (opcion !== "mascota") {
      return interaction.editReply({ content: `${bostezo}Esa opción todavía no está disponible.` });
    }
    ...
    if (etiquetas <= 0) {
      return interaction.editReply({
        content: `${bostezo}Para renombrar necesitas una **Etiqueta para Mascota**. La puedes comprar en **/tienda** y usar con **/comprar item:etiqueta_mascota**.`,
      });
    }
    ...
    return interaction.editReply({
      content: `🏷️ ¡Listo! Tu mascota **${base}** ahora se llama **${nuevoNombre}**. Se consumió **1 Etiqueta para Mascota**.`,
    });
  } catch (error) {
